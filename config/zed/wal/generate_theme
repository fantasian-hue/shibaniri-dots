#!/usr/bin/env bash
# pywal-to-zed.sh - convert pywal theme to zed text editor theme
#
# originally written by `Fuwn` for their `zed-theme-wal` repository
#
# i've forked their repo and made this version to ensure the resulting theme file
# ends up in the original zed configuration location, so that the theme updates
# automatically instead of having to manually refresh/reload.
#
# Fuwn's original version of the script can still be found in this repository if you'd like; named `generate_theme`
